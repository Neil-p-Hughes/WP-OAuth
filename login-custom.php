...
define('SCOPE', get_option('wpoa_custom_scope')); // PROVIDER SPECIFIC: 'wl.basic' is the minimum scope required to get the user's id from Windows Live
define('URL_AUTH', get_option('wpoa_custom_url_auth'));
define('URL_TOKEN', get_option('wpoa_custom_url_token'));
define('URL_USER', get_option('wpoa_custom_url_user'));
...
elseif (!CLIENT_ID || !CLIENT_SECRET || !SCOPE || !URL_AUTH || !URL_TOKEN || !URL_USER) {
	// do not proceed if id or secret is null:
	$this->wpoa_end_login("This third-party authentication provider has not been configured with required API settings. Please notify the admin or try again later.");
}
...
elseif (isset($_GET['code'])) {
	...
	if (!isset($_GET['state']) || $_SESSION['WPOA']['STATE'] == $_GET['state']) {
...
function get_oauth_identity($wpoa) {
	...
	$oauth_identity['id'] = $result_obj['sub']; // PROVIDER SPECIFIC: this is how Windows Live returns the user's unique id
	$oauth_identity['email'] = $result_obj['email']; // PROVIDER SPECIFIC: Windows Live never provides the user's email!

	$oauth_identity['user_nicename'] = $result_obj['given_name'] . " " . $result_obj['family_name'][0]; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
	$oauth_identity['display_name'] = $result_obj['given_name'] . " " . $result_obj['family_name'][0]; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
	$oauth_identity['nickname'] = $result_obj['given_name'] . " " . $result_obj['family_name'][0]; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
	$oauth_identity['user_email'] = $result_obj['email']; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
	$oauth_identity['username'] = $result_obj['preferred_username']; // PROVIDER SPECIFIC: Windows Live never provides the user's email!
