...
$nickname = $username;
$user_email = "";
if (is_array($_SESSION["WPOA"]["OAUTH_IDENTITY"])) {
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["username"])) {
		$username = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["username"];
	}
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["user_nicename"])) {
		$user_nicename = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["user_nicename"];
	}
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["display_name"])) {
		$display_name = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["display_name"];
	}
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["nickname"])) {
		$nickname = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["nickname"];
	}
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["nickname"])) {
		$nickname = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["nickname"];
	}
	if ( isset($_SESSION["WPOA"]["OAUTH_IDENTITY"]["user_email"])) {
		$user_email = $_SESSION["WPOA"]["OAUTH_IDENTITY"]["user_email"];
	}

}
if ($user_email != "") {
	$update_username_result = $wpdb->update($wpdb->users, array('user_email' => $user_email, 'user_login' => $username, 'user_nicename' => $user_nicename, 'display_name' => $display_name), array('ID' => $user_id));
}
else {
	$update_username_result = $wpdb->update($wpdb->users, array('user_login' => $username, 'user_nicename' => $user_nicename, 'display_name' => $display_name), array('ID' => $user_id));
}
